...
======================================================================
Phase 1 (from code1.py): Cross-validated logistic regression learns optimal
    weights for 7 spatial similarity metrics using known positive/negative controls.
Phase 2 (from code234.py): 3-stage isotope detection pipeline uses the
    calibrated weights for spatial pattern matching → identification → hierarchy.

BUG FIXES (v2):
    1. imp_iou division-by-zero in get_8_metrics — added + 1e-8 to denominator
    2. Moran's I was hardcoded to 0.5 in calibration — now actually computed
    3. value_iou was never computed in compute_coordinate_based_similarity — now included
    4. Importance computation mismatch between calibration and pipeline — unified
    5. Coordinate normalization mismatch in calibration k-NN — now consistent
    6. Histogram computation mismatch (np.histogram2d vs compute_spatial_histogram) — unified
    7. FOLD_CHANGE renamed to N_FOLDS for clarity

CLEANUP (v3):
    8. Removed unused `coords` parameter from compute_morans_i_vectorized
    9. Removed radial_profile entirely (unused: function, dataclass field, all references)
   10. get_sig now uses its `coords_normalized` parameter instead of re-extracting raw coords
   11. Removed unused `grid_res` parameter from compute_coordinate_based_similarity
   12. Removed redundant classify_mass_difference call in compute_pair_similarity
   13. Fixed misleading mz_1_rounded/mz_2_rounded column names in identify_isotopes

BUG FIXES & CLEANUP (v4):
   14. value_correlation mismatch: calibration used ALL pixels, pipeline filtered
       to non-zero — unified to filter non-zero in both (Pearson r, range [-1, 1])
   15. importance_iou mismatch: calibration used binary Jaccard (threshold 0.5),
       pipeline used continuous soft IoU (max-normalized) — unified to continuous
       soft IoU in both (range [0, 1])
   16. Removed Moran's I similarity entirely: function, dataclass field, feature
       list, all computation and scoring references. Reduced from 8 to 7 metrics.
   17. All 7 metrics now produce values in [-1, 1]. max(0, ...) is applied at
       scoring time to use only the positive contribution of each metric.
"""
...
# Output directory
OUTPUT_DIR = './mz_isotope_results_v5'
...
# =============================================================================
# CALIBRATION CONFIGURATION (from code1.py)
# =============================================================================
...
# Ordered feature names shared by calibration and scoring (7 metrics, Moran's I removed)
ORDERED_FEATURES = [
...
# =============================================================================
# CALIBRATION: CROSS-VALIDATED ML CALIBRATOR (from code1.py)
# =============================================================================
...
def get_7_metrics(s1: CalibrationSignature, s2: CalibrationSignature) -> dict:
    """
    Compute 7 spatial similarity metrics between two calibration signatures.
    All metrics produce values in [-1, 1]. max(0, ...) is applied at scoring time.

    Metrics and their natural ranges:
        intensity_ratio_consistency : [0, 1]   — 1/(1+CV), always non-negative
        value_correlation           : [-1, 1]  — Pearson r on non-zero pixels
        peak_colocalization         : [0, 1]   — Jaccard index of top-20% pixels
        importance_iou              : [0, 1]   — continuous soft IoU (min/max)
        importance_correlation      : [-1, 1]  — Pearson r of importance scores
        spatial_hist_corr           : [-1, 1]  — Pearson r of spatial histograms
        value_iou                   : [0, 1]   — continuous soft IoU of raw values
    """
    # 1. Intensity Ratio Consistency [0, 1]
    mask_irc = (s1.values > np.percentile(s1.values, 10)) & (s2.values > np.percentile(s2.values, 10))
    r_con = 0.0
    if mask_irc.sum() > 10:
        rats = np.minimum(s1.values[mask_irc], s2.values[mask_irc]) / (
            np.maximum(s1.values[mask_irc], s2.values[mask_irc]) + 1e-8)
        r_con = 1 / (1 + (np.std(rats) / (np.mean(rats) + 1e-8)))

    # 2. Value Correlation [-1, 1] — FIX 14: filter to non-zero pixels (matches pipeline)
    mask_nz = (s1.values > 0) | (s2.values > 0)
    v_corr = 0.0
    if mask_nz.sum() > 10:
        r, _ = pearsonr(s1.values[mask_nz], s2.values[mask_nz])
        v_corr = r if not np.isnan(r) else 0.0

    # 3. Peak Colocalization [0, 1]
    p1 = s1.values >= np.percentile(s1.values, 80)
    p2 = s2.values >= np.percentile(s2.values, 80)
    peak = (p1 & p2).sum() / ((p1 | p2).sum() + 1e-8)

    # 4. Importance IoU [0, 1] — FIX 15: continuous soft IoU (matches pipeline)
    ...
    # 5. Importance Correlation [-1, 1]
    i_corr = 0.0
    if len(s1.importance) > 10:
        r, _ = pearsonr(s1.importance, s2.importance)
        i_corr = r if not np.isnan(r) else 0.0

    # 6. Spatial Histogram Correlation [-1, 1]
    h1, h2 = s1.histogram.flatten(), s2.histogram.flatten()
    h_corr = 0.0
    if h1.std() > 0 and h2.std() > 0:
        r, _ = pearsonr(h1, h2)
        h_corr = r if not np.isnan(r) else 0.0

    # 7. Value IoU [0, 1]
    val_iou = np.minimum(s1.values, s2.values).sum() / (
        np.maximum(s1.values, s2.values).sum() + 1e-8)
...
class CrossValidatedCalibrator:
    ...
    def get_sig(self, adata, mz, nn_idx, coords_normalized):
        ...
        return CalibrationSignature(mz, v, hist, imp)
...
# =============================================================================
# STAGE 1: SPATIAL PATTERN MATCHING FUNCTIONS (from code234.py)
# =============================================================================
...
def compute_coordinate_based_similarity(sig1: SpatialSignature, sig2: SpatialSignature) -> dict:
    ...
    All metrics produce values in [-1, 1]. max(0, ...) is applied at scoring time.
    """
    # FIX 14: filter to non-zero pixels (matches calibration)
    mask = (sig1.raw_values > 0) | (sig2.raw_values > 0)
    value_corr = 0.0
    if mask.sum() > 10:
        r, _ = pearsonr(sig1.raw_values[mask], sig2.raw_values[mask])
        value_corr = r if not np.isnan(r) else 0.0

    imp_corr = 0.0
    if len(sig1.node_importance) > 10:
        r, _ = pearsonr(sig1.node_importance, sig2.node_importance)
        imp_corr = r if not np.isnan(r) else 0.0

    # FIX 15: continuous soft IoU with max-normalization (matches calibration)
...
def compute_descriptor_similarity(sig1: SpatialSignature, sig2: SpatialSignature) -> dict:
    """Compute descriptor-based similarity. Returns spatial_hist_corr in [-1, 1]."""
    h1, h2 = sig1.spatial_histogram.flatten(), sig2.spatial_histogram.flatten()
    spatial_hist_corr = 0.0
    if h1.std() > 0 and h2.std() > 0:
        r, _ = pearsonr(h1, h2)
        spatial_hist_corr = r if not np.isnan(r) else 0.0
...
def compute_combined_score(coord_sim: dict, desc_sim: dict, weights: dict) -> float:
    ...
    max(0, ...) ensures only positive similarity contributions are used.

    Parameters:
    -----------
    coord_sim : dict — Coordinate-based similarity metrics.
    desc_sim : dict — Descriptor-based similarity metrics.
    weights : dict — Calibrated weights from CrossValidatedCalibrator (metric_name → weight).
    """
    score = (
    ...
    def run_analysis(self):
        ...
        for sample_id in tqdm(MSI_SAMPLE_IDS, desc="Samples", unit="sample"):
            ...
            def extract_single_mz(i):
                return i, self.extract_signature(msi_coords, msi_data[:, i], sample_id, mz_names[i], nn_indices)
...
# =============================================================================
# STAGE 2: ISOTOPE IDENTIFICATION FUNCTIONS (from code234.py)
# =============================================================================
...
def identify_isotopes(df, min_animals=12, min_score=60):
    ...
    print(f"\nUnique mz_1 values: {df['mz_1'].nunique()}")
    print(f"Unique mz_2 values: {df['mz_2'].nunique()}")
    ...
    df_filtered['pair_id'] = df_filtered.apply(
        lambda row: f"{row['mz_1']:.3f}_{row['mz_2']:.3f}_{row['mass_diff_type']}",
        axis=1
    )
...
# =============================================================================
# STAGE 3: PARENT-CHILDREN HIERARCHY (from code234.py)
# =============================================================================
...
def main():
    ...
    return calibrated_weights, matcher, matching_results, isotope_results, hierarchy_results
