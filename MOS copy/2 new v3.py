...
Phase 1 (from code1.py): Cross-validated logistic regression learns optimal
    weights for 9 spatial similarity metrics using known positive/negative controls.
...
CODE QUALITY FIXES (v3):
    8.  get_sig now uses coords_normalized parameter instead of re-extracting
        raw coords from adata.obs — consistent normalized coordinates throughout
    9.  Removed unused coords parameter from compute_morans_i_vectorized
    10. radial_profile is now used: added radial_profile_corr as 9th metric
        in both calibration (get_9_metrics) and pipeline (compute_descriptor_similarity)
    11. Removed unused grid_res parameter from compute_coordinate_based_similarity
    12. mz_1_rounded / mz_2_rounded now actually round to 3 decimal places
    13. Removed redundant classify_mass_difference call in compute_pair_similarity
        (mass_diff_type is overwritten by run_analysis anyway)
"""
...
# Output directory
OUTPUT_DIR = './mz_isotope_results_v3'
...
# FIX 10: Now 9 features — radial_profile_corr added
ORDERED_FEATURES = [
...
class CalibrationSignature:
    """Lightweight signature used only during weight calibration (from code1.py)."""
    mz: str
    values: np.ndarray
    histogram: np.ndarray
    importance: np.ndarray
    morans: float
    radial_profile: np.ndarray  # FIX 10: added radial profile
...
def get_9_metrics(s1: CalibrationSignature, s2: CalibrationSignature) -> dict:
    ...
    # FIX 10: Radial profile correlation
...
class CrossValidatedCalibrator:
    ...
    def get_sig(self, adata, mz, nn_idx, coords_normalized):
        ...
        FIX 8: Now uses coords_normalized directly instead of re-extracting
        raw coords from adata.obs. All spatial computations (histogram,
        radial profile, Moran's I) use the same normalized coordinate space
        as k-NN neighbor assignment.
        """
        ...
        # FIX 9: compute_morans_i_vectorized no longer takes coords
        morans = compute_morans_i_vectorized(v, nn_idx)

        # FIX 10: compute radial profile for calibration
...
def compute_morans_i_vectorized(values: np.ndarray, indices: np.ndarray) -> float:
    ...
    FIX 9: Removed unused coords parameter — only values and neighbor
    indices are needed for the computation.
    """
    n = len(values)
...
def compute_coordinate_based_similarity(sig1: SpatialSignature, sig2: SpatialSignature) -> dict:
    ...
    FIX 11: Removed unused grid_res parameter.
    """
...
def compute_descriptor_similarity(sig1: SpatialSignature, sig2: SpatialSignature) -> dict:
    ...
    FIX 10: Now includes radial_profile_corr using the radial profiles
    that were already computed in extract_signature.
    """
    def safe_pearsonr(a, b):
        if a.std() > 0 and b.std() > 0:
            r, _ = pearsonr(a, b)
...
class MzIsotopeMatcher:
    ...
    def extract_signature(self, coords: np.ndarray, values: np.ndarray, sample_id: str,
        ...
        return SpatialSignature(
            ...
            morans_i=compute_morans_i_vectorized(values, nn_indices),  # FIX 9: no coords arg
    ...
    def compute_pair_similarity(self, sig1: SpatialSignature, sig2: SpatialSignature) -> dict:
        ...
        FIX 11: Removed grid_res from compute_coordinate_based_similarity call.
        FIX 13: Removed redundant classify_mass_difference call — mass_diff_type
        is overwritten by compute_pair in run_analysis anyway.
        """
...
def identify_isotopes(df, min_animals=12, min_score=60):
    ...
    # FIX 12: Actually round to 3 decimal places (matching the .3f in pair_id)
    df['mz_1_rounded'] = df['mz_1'].round(3)
    df['mz_2_rounded'] = df['mz_2'].round(3)
