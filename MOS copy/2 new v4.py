...
Phase 1 (from code1.py): Cross-validated logistic regression learns optimal
    weights for 8 spatial similarity metrics using known positive/negative controls.
...
# Output directory
OUTPUT_DIR = './mz_isotope_results_v4'
...
# Ordered feature names shared by calibration and scoring
ORDERED_FEATURES = [
...
def get_8_metrics(s1: CalibrationSignature, s2: CalibrationSignature) -> dict:
...
class CrossValidatedCalibrator:
    ...
    # FIX 10: get_sig now uses coords_normalized instead of re-extracting raw coords
    def get_sig(self, adata, mz, nn_idx, coords_normalized):
        ...
        # FIX 8: coords parameter removed from compute_morans_i_vectorized
        morans = compute_morans_i_vectorized(v, nn_idx)

        return CalibrationSignature(mz, v, hist, imp, morans)
...
# FIX 8: Removed unused `coords` parameter — only `values` and `indices` are needed
...
# FIX 9: Removed radial_profile field from SpatialSignature
@dataclass
...
def compute_combined_score(coord_sim: dict, desc_sim: dict, weights: dict) -> float:
    ...
    desc_sim : dict
        Descriptor-based similarity metrics.
    weights : dict
...
class MzIsotopeMatcher:
    ...
    # FIX 9: Removed radial_profile from signature extraction
    ...
    # FIX 12: Removed redundant classify_mass_difference call — mass_diff_type is
    #         always overwritten by the caller (compute_pair in run_analysis)
...
def identify_isotopes(df, min_animals=12, min_score=60):
    ...
    # FIX 13: Removed misleading mz_1_rounded/mz_2_rounded aliases — the pair_id
    # f-string already truncates to 3 decimal places for grouping purposes
