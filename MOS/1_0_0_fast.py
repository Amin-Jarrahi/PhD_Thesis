"""
Gene-to-m/z Spatial Pattern Matching Pipeline — SPEED-OPTIMIZED VERSION
=======================================================================
"""
...
from scipy.interpolate import griddata, LinearNDInterpolator  # ◆ SPEED: added
from scipy.spatial import Delaunay  # ◆ SPEED: for reusable triangulation
...
class AnalyticPatternMatcher:
    ...
    def precompute_msi_cache(self, msi_sample: str):
        ...
        for i, mz_name in enumerate(mz_names):
            ...
            sig = SpatialSignature(
                ...
                morans_i=compute_morans_i_vectorized(msi_coords, vals, msi_nn_indices),
            ...
            morans_arr.append(sig.morans_i)
        ...
        self._msi_descriptor_stack[msi_sample] = {
            ...
            'morans': np.array(morans_arr),            # (n_mz,)
            'grid_x': grid_x,
            'grid_y': grid_y,
        }
    ...
    def find_matches_fast(self, gene_sig: SpatialSignature, msi_sample: str, top_k: int = 20) -> pd.DataFrame:
        """
        Vectorized matching: compute all 8 metrics for all m/z features
        ...
        morans_sims = 1.0 - np.abs(gene_sig.morans_i - stack['morans'])
        ...
        combined_scores = (
            ...
            w['morans_similarity']      * np.maximum(morans_sims, 0)
        )
    ...
    def run_analysis(self, top_k=20):
        print("\n" + "="*70)
        print("GENE-TO-M/Z MATCHING V1 (Analytic - Speed Optimized)") 
...
def main():
    print("="*70)
    print("V1: Analytic Spatial Matching (No GNN/Transformer) - Speed Optimized") 
