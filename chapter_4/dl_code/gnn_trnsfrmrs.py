"""
MSI-ST Hybrid Model Pipeline (Improved Version)
File: MSI_ST_Hybrid_Model_Pipeline_v2.py

Key Improvements:
- Fixed transformer architecture for proper sequence modeling
- Proper batch handling with dynamic graphs
- Added positional encodings
- Memory-efficient training with gradient accumulation
- Better data normalization and preprocessing
- Comprehensive logging and validation
- Flexible alignment strategies (contrastive + optional CCA/OT)
...
class MSITransformerEncoder(nn.Module):
    ...
    def __init__(self, n_mz_bins: int, d_model: int = 256, n_heads: int = 8,
        ...
        self.pos_encoder = PositionalEncoding(d_model, max_len=self.n_patches, dropout=dropout)
